...
const searchEntities = entities.filter((entry) => entry.slug !== 'llm-map');
mkdirSync(generatedRoot, { recursive: true });
buildSymbols(projectRoot, entities);
writeFileSync(
  join(generatedRoot, 'search-index.ts'),
  `// Generated by scripts/content/build-derived.mjs. Do not edit.\nexport const searchIndex = ${JSON.stringify(searchEntities.map(({ markdown, ...entry }) => entry), null, 2)} as const;\n`,
  'utf8'
);
