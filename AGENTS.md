...
- Load `SearchDialog.svelte` only after the user opens search. Production search must use Pagefind and its language shards; the multi-megabyte generated search index and symbol index are development fallbacks or dedicated-catalog data and must never enter an initial production preload.
