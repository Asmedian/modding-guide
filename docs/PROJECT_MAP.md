...
- `/` chooses a saved locale in an inline head script before the body paints; first visit uses English only when the browser language begins with `en`, otherwise Russian. With JavaScript disabled, the otherwise empty route exposes direct RU/EN links.
