...
The central trigger/receiver strip uses the corresponding registries. Table and constant/global links in an ERM article open in the right reference panel; further internal ERM links stay there. Back/forward history, scroll positions, and mouse buttons 4/5 belong to that panel. Normal links remain readable without JavaScript. Keep the learning route heading-only.
